...
// equals()
// Returns true if A is same int sequence as B, false otherwise.  bool equals(List A, List B){
...
void deleteFront(List L){
   ...
   if( length(L)>1 ) { 
      ...
      L->front = L->back = NULL; // if list is only length 1, make empty by setting front and back to null
   }
   L->length--;
   freeNode(&N);
}
...
void delete(List L){ 
   ...
   if(L->front == L->cursor){//checks if cursor is at front, performs deleteFront if true
       deleteFront(L);
   }else if(L->back == L->cursor){//checks if cursor is at back, performs deleteBack if true
       deleteBack(L);
   }else{
